...
# Sub-menu based on main menu selection - 새로운 메뉴 목록으로 업데이트
if main_menu == "3.1 Siddhi Shrestha":
    page = st.sidebar.radio(
        ...
        [
            ...
            "3.1.6 GEE Land Cover Analysis 🛰️", # 새로운 지표 추가
            "3.1.7 Humanitarian water support ", 
            "3.1.8 Humanitarian sanitation & hygiene ", # 인디케이터 이름 변경
            "3.1.9 End Year Progress against Annual target " # 인디케이터 이름 변경
        ]
    )
elif main_menu == "3.2 Dandi Ram":
    # Dandi Ram 메뉴는 이전 코드의 "Coming Soon..." 유지
    page = st.sidebar.radio(
        "Select Indicator:",
        ["Coming Soon..."]
    )
...
# 'End Year Progress against Annual target'은 메인 메뉴의 '3.1 Siddhi Shrestha' 외의 다른 지표로 처리될 수 있도록, 별도 page 변수 할당은 건너뜁니다.
elif main_menu == "End Year Progress against Annual target":
    page = "End Year Progress against Annual target"
...
def create_nepal_map(office_df, palika_df):
    ...
    # Add Marker Cluster to manage many palika markers if we decide to add them later
    # marker_cluster = plugins.MarkerCluster().add_to(nepal_map) 
...
try:
    ...
    if main_menu == "3.1 Siddhi Shrestha":
        # Check if the selected indicator is one of the "Coming Soon" ones or the fully implemented one
        if page == "3.1.1 Safe water access 🚰":
            ...
            if view_mode == "📊 Office Summary Dashboard":
                ...
                # Note: This list comprehension needs OFFICE_COORDINATES keys to match plot_df['Office'] exactly
                office_colors_map = {k: v['color'] for k, v in OFFICE_COORDINATES.items()}
                office_colors = [office_colors_map.get(office, 'gray') for office in plot_df['Office']]
            ...
            # --- PAGE 2: Nepal Map & Palika Analysis (NEW) ---
                ...
                with tab1:
                    ...
                    for idx, row in plot_df.iterrows():
                        if idx < len(cols):
                            with cols[idx]:
                                office_name = row['Office']
                                # Ensure office_name is a valid key before accessing
                                if office_name in OFFICE_COORDINATES:
                                    color = OFFICE_COORDINATES[office_name]['color']
                                else:
                                    color = 'gray' # Default color if key is missing
                ...
                with tab2:
                    ...
                    # Recreate office_colors for safety
                    office_colors = [office_colors_map.get(office, 'gray') for office in plot_df['Office']]
                ...
                with tab3:
                    ...
                    if len(top_10) > 0:
                        ...
                        # Fix for Matplotlib 3.1+ deprecated function usage
        ...
        elif page in ["3.1.2 Water-safe communities 💧", "3.1.3 Basic sanitation gained 🚽", 
                      "3.1.4 Schools with WASH 🏫", "3.1.5 HCFs with WASH 🏥", 
                      "3.1.6 GEE Land Cover Analysis 🛰️", "3.1.7 Humanitarian water support ", 
                      "3.1.8 Humanitarian sanitation & hygiene ", "3.1.9 End Year Progress against Annual target "]:
            # Other indicators under Siddhi Shrestha - show "Ongoing"
            display_coming_soon(page)
        
        else: # Fallback for any unlisted/old pages
             display_coming_soon(page)
