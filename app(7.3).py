...
# -*- coding: utf-8 -*-
# app.py (Streamlit WASH Dashboard) - reference: 6.8 baseline + requested nav updates
...
# -------------------------------------------
# Constants / Assumptions (as 6.8 baseline)
# -------------------------------------------
SAN_BENEFICIARY_PER_TOILET = 5
SANITATION_TARGETS = {
...
# -------------------------------------------
# Sidebar Navigation (requested updates)
# -------------------------------------------
st.sidebar.title("🧭 Navigation")

# ✅ 요청사항 1: Select Team 라벨 단순화(3.3 Arinita - FO → FO 단일 라벨)
main_menu = st.sidebar.selectbox(
    ...
    [
        ...
        "3.2 Dandi Ram",
        "3.3 Arinita",
        # Field Offices moved as plain names
        "Janakpur",
        "Dhangadi",
        "Bhairahawa",
        "Surkhet",
...
if main_menu == "3.1 Siddhi Shrestha":
    ...
    # ✅ 요청사항 2: Dandi Ram 섹션에 지표 2개 추가
    dandi_page = st.sidebar.radio("Select Indicator:", ["LGPAS", "Palikas"])

elif main_menu in ["3.3 Arinita", "Janakpur", "Dhangadi", "Bhairahawa", "Surkhet"]:
    # 선택된 Field Office (6.8 스타일의 단순 파싱)
    selected_fo = None
    if main_menu in ["Janakpur", "Dhangadi", "Bhairahawa", "Surkhet"]:
        selected_fo = main_menu

    # ✅ 요청사항 4: Arinita 인디케이터 목록 수정
    arinita_indicator = st.sidebar.radio(
        "Select Indicator:",
        ["Flood Map 2024", "Cholera Outbreak 2025", "Groundwater Monitoring"]
    )

st.sidebar.markdown("---")

# -------------------------------------------
# Sidebar - File Configuration (6.8 유지)
# -------------------------------------------
...
# -------------------------------------------
# Field Office Coordinates (6.8 유지)
# -------------------------------------------
OFFICE_COORDINATES = {
...
def ensure_columns(df: pd.DataFrame, required: list):
    ...
    if missing:
        raise KeyError(
            "필요 컬럼 누락: " + ", ".join(missing)
            + "\n현재 CSV 컬럼(일부): " + ", ".join(list(df.columns)[:30])
        )
...
# -------------------------------------------
# Processing: 3.1.2 Water-safe communities (6.8 유지)
# -------------------------------------------
@st.cache_data
def process_office_data_312(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for key, info in office_mapping.items():
        ...
        df_f = df[cond_office & cond_water_safe & cond_2025]
        total = df_f[total_col].sum()
        ach = (total / info['target'] * 100) if info['target'] > 0 else 0.0
        rows.append({'Office': info['name'], 'Beneficiaries': int(total), 'Target': info['target'], 'Achievement': ach})
    out = pd.DataFrame(rows)
    return out[out['Target'] > 0].copy()
...
def process_palika_data_312(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cond_water_safe = df[wsc_col].str.contains(r'\b(yes|y)\b', na=False)
    cond_2025 = df[wsc_year_col] == 2025
    df_f = df[cond_water_safe & cond_2025].copy()

    office_map = {'nco': 'NCO', 'janakpur': 'Janakpur', 'dhangadi': 'Dhangadi', 'bhairahawa': 'Bhairahawa', 'surkhet': 'Surkhet'}
    ...
    df_f['Office'] = df_f[office_col].apply(map_office)
    palika_summary = (
        df_f.groupby(['Office', palika_col, district_col, province_col])[total_col]
            .sum()
            .reset_index()
    )
    ...
    palika_summary = palika_summary[palika_summary['Office'] != 'Unknown'].sort_values('Beneficiaries', ascending=False)
    return palika_summary

# -------------------------------------------
# Processing: 3.1.1 Safe water access (6.8 유지)
# -------------------------------------------
@st.cache_data
def process_office_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for key, info in office_mapping.items():
        cond_office = df[office_col].str.contains(rf'\b{re.escape(key)}\b', na=False)
        df_f = df[cond_office & cond_completed & cond_safe & cond_2025]
        total = df_f[total_col].sum()
        ach = (total / info['target'] * 100) if info['target'] > 0 else 0.0
        rows.append({'Office': info['name'], 'Beneficiaries': int(total), 'Target': info['target'], 'Achievement': ach})
    out = pd.DataFrame(rows)
...
def process_palika_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cond_safe = df[wq_col].str.contains(r'\b(yes|y)\b', na=False)
    cond_2025 = df[year_col] == 2025
    df_f = df[cond_completed & cond_safe & cond_2025].copy()
...
# -------------------------------------------
# Processing: 3.1.3 Basic sanitation gained (6.8 유지)
# -------------------------------------------
@st.cache_data
def process_office_data_313(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for key, info in SANITATION_TARGETS.items():
        cond_office = df[office_col].str.contains(rf'\b{re.escape(key)}\b', na=False)
        df_f = df[cond_office & cond_completed & cond_2025]
        total = df_f['_beneficiaries_313'].sum()
        ach = (total / info['target'] * 100) if info['target'] > 0 else 0.0
        rows.append({'Office': info['name'], 'Beneficiaries': int(round(total)), 'Target': info['target'], 'Achievement': ach})
    out = pd.DataFrame(rows)
    return out[out['Target'] > 0].copy()
...
def process_palika_data_313(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cond_completed = df[progress_col].str.contains(r'\bcompleted\b', na=False)
    cond_2025 = df[year_col] == 2025
    df_f = df[cond_completed & cond_2025].copy()
    ...
    df_f['Office'] = df_f[office_col].apply(map_office)
    palika_summary = (
        df_f.groupby(['Office', palika_col, district_col, province_col])['_beneficiaries_313']
            .sum()
            .reset_index()
    )
    palika_summary.columns = ['Office', 'Palika', 'District', 'Province', 'Beneficiaries']
...
def create_nepal_map(office_df: pd.DataFrame, palika_df: pd.DataFrame):
    ...
    for _, row in office_df.iterrows():
        ...
        folium.CircleMarker(
            ...
            color=color,
            fill=True, fillColor=color, fillOpacity=0.6, weight=1
        ).add_to(nepal_map)
...
try:
    if main_menu == "3.1 Siddhi Shrestha":
        ...
        # 선택 지표 표시 (우선 커밍순)
        display_coming_soon(f"3.2 Dandi Ram - {dandi_page}")

    elif main_menu in ["3.3 Arinita", "Janakpur", "Dhangadi", "Bhairahawa", "Surkhet"]:
        _fo = selected_fo if 'selected_fo' in locals() and selected_fo else "All Field Offices"
        _ind = arinita_indicator if 'arinita_indicator' in locals() else "Indicator not selected"
        display_coming_soon(f"3.3 Arinita - {_fo} - {_ind}")

    elif main_menu == "Progress against target":
        display_coming_soon("Progress against target")

    # Footer / Filters info (6.8 유지)
    if main_menu == "3.1 Siddhi Shrestha":
        if page == "3.1.1 Safe water access 🚰":
    ...
    st.info("💡 **해결 방법:**")
    st.write("1. `WASH.csv` 파일이 올바른 위치에 있는지 확인하세요 (예: 현재 디렉토리 또는    st.write("1. `WASH.csv` 파일이 올바른 위치에 있는지 확인하세요 (예: 현재 디렉토리 또는 data 폴더)")
    ...
    st.info(
        "💡 **해결 방법:** CSV 헤더(컬럼명)를 확인하고, 데이터 컬럼명이 본 코드의 기대 표준명과 크게 다를 경우 "
        "상단의 'CSV 컬럼 확인(디버그)' 옵션을 켜서 실제 컬럼명을 확인한 뒤 매핑 사전에 변형명을 추가해 주세요."
    )

except Exception as e:
    # ✅ 반드시 들여쓰기된 블록을 제공해 IndentationError 방지
