...
# app.py (Streamlit WASH Dashboard) - Robust Column Mapping & Fixes
    ...
    for std_name, variants in mapping.items():
        ...
        for v in variants:
            ...
            if nv in current_cols:
                ...
                break
        # 2) 변형 목록에 없으면 느슨한 탐색(부분일치/물음표 유무)
        if not target:
...
def load_data(path):
    ...
    df = robust_rename_columns(df, {
        ...
        'community declared water safe?': [
            'community declared water safe?',
            'community declared water safe',
            'is community declared water safe?',
            'community declared watersafe?',
            'wsc confirmed water safe?',
            'community water safe?'
        ],
        'wsc reporting year': [
            'wsc reporting year',
            'water safe community reporting year',
            'wsc (reporting year)',
            'reporting year (wsc)',
            'year (wsc)'
        ],
...
def process_office_data_312(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 모든 오피스 표시(타겟>0). Beneficiaries는 0도 허용.
...
try:
    if main_menu == "3.1 Siddhi Shrestha":
        if page == "3.1.1 Safe water access 🚰":
            ...
            if view_mode == "📊 Office Summary Dashboard":
                ...
                # Row 2: Target vs Achievement + table
                col1, col2 = st.columns(2)
                with col1:
    ...
    # For deeper debugging, uncomment below two lines:
    # import traceback
