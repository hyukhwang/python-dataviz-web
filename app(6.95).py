...
# app.py (Streamlit WASH Dashboard) - based on app(6.8).py with Select Team adjustments
...
# -------------------------------------------------
# Sidebar Navigation (UPDATED per request)
# -------------------------------------------------
...
main_menu = st.sidebar.selectbox(
    ...
    [
        ...
        "3.2 Dandi Ram",
        "3.3 Arinita",
        # ▼ Arinita FO: 요청대로 단순 표기 (중복 방지 위해 Surkhet 유지)
        "Janakpur",
        "Dhangadi",
        "Bhairahawa",
        "Surkhet",
...
# 3.3 Arinita 및 FO: 선택 시 Arinita 인디케이터 라디오 표시
elif main_menu in ["3.3 Arinita", "Janakpur", "Dhangadi", "Bhairahawa", "Surkhet"]:
    selected_fo = None if main_menu == "3.3 Arinita" else main_menu
...
# -------------------------------------------------
# Main app logic (원본 흐름 유지, Select Team만 조정)
# -------------------------------------------------
try:
    if main_menu == "3.1 Siddhi Shrestha":
        # -------------------- 3.1.1 --------------------
            ...
            if view_mode == "📊 Office Summary Dashboard":
                ...
                with tab2:
                    ...
                    with col1:
                        ...
                        ax1.bar(plot_df['Office'], plot_df['Beneficiaries'], color=colors, edgecolor='black', linewidth=1.5)
                        ax1.set_ylabel('Total Beneficiaries', fontsize=12, fontweight='bold')
                        ax1.grid(axis='y', alpha=0.3)
                        plt.tight_layout()
                        st.pyplot(fig1)
                    with col2:
                        ...
                        fig2, ax2 = plt.subplots(figsize=(8, 6))
                        x = range(len(plot_df)); width = 0.35
                        ...
                        ax2.set_xticks(list(x)); ax2.set_xticklabels(plot_df['Office'], fontsize=10)
                ...
                with tab3:
                    ...
                    if len(top_10) > 0:
                        ...
                        ax3.set_yticks(list(range(len(palika_labels)))); ax3.set_yticklabels(palika_labels)
                        for b in bars:
                            w = b.get_width()
        ...
        # -------------------- 3.1.3 --------------------
            ...
            if view_mode == "📊 Office Summary Dashboard":
                ...
                with col1:
                    ...
                    ax3.set_ylabel('Number of Beneficiaries', fontsize=12, fontweight='bold')
                    ax3.set_title('Target vs Achievement', fontsize=14, fontweight='bold')
                    ax3.set_xticks(list(x)); ax3.set_xticklabels(plot_df['Office'], fontsize=10)
    ...
    # 3.3 Arinita & FO (단순 표기 선택 시 Indicator를 제목에 반영)
    elif main_menu in ["3.3 Arinita", "Janakpur", "Dhangadi", "Bhairahawa", "Surkhet"]:
    ...
    st    st.error(f"❌ Error loading data or rendering dashboard: {str(e)}")
    # import traceback
