...
# app.py (Streamlit WASH Dashboard) - based on app(6.8).py with minimal nav updates
# Author: Hyeok Hwang + Copilot
# Last update: 2025-12-17  # 🔧 UPDATE: date bump
import os
import re
...
# 🔧 UPDATE: Select Team 항목에서 3.3 Arinita-* 네 개를 단순 지명으로 변경
main_menu = st.sidebar.selectbox(
    ...
    [
        ...
        "3.2 Dandi Ram",
        "3.3 Arinita",
        # ▼ Field Office 4개를 Select Team에서 단순화
        "Janakpur",
        "Dhangadi",
        "Bhairahawa",
        "Surkhet",
...
if main_menu == "3.1 Siddhi Shrestha":
    ...
    # 🔧 UPDATE: 요청한 2개 인디케이터 추가
    dandi_page = st.sidebar.radio("Select Indicator:", ["LGPAS", "Palikas"])

elif main_menu in [
    "3.3 Arinita",
    # 🔧 UPDATE: 단순 지명 4개도 같은 분기에서 처리
    "Janakpur",
    "Dhangadi",
    "Bhairahawa",
    "Surkhet"
]:
    # 선택된 Field Office 파싱 (기본: None)
    selected_fo = None
    # 🔧 UPDATE: 단순 지명 선택 시 해당 값을 FO로 사용
    if main_menu in ["Janakpur", "Dhangadi", "Bhairahawa", "Surkhet"]:
        selected_fo = main_menu
    elif " - " in main_menu:
        selected_fo = main_menu.split(" - ", 1)[1].strip()

    # 🔧 UPDATE: Arinita 인디케이터 목록 수정 (Cholera Outbreak 2025)
...
# --------------------------------------------------
# Nepal Field Office Coordinates (원본 유지)
# --------------------------------------------------
OFFICE_COORDINATES = {
...
# --------------------------------------------------
# Helpers: Normalize & robust column resolver (원본 유지)
# --------------------------------------------------
...
# --------------------------------------------------
# Processing: 3.1.2 (Water-safe communities) (원본 유지)
# --------------------------------------------------
@st.cache_data
...
# --------------------------------------------------
# Processing: 3.1.1 (Safe water access) (원본 유지)
# --------------------------------------------------
@st.cache_data
...
# --------------------------------------------------
# Processing: 3.1.3 (Basic sanitation gained) (원본 유지)
# --------------------------------------------------
@st.cache_data
def process_office_data_313(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Filters mirror 3.1.1/3.1.2 (completed + year 2025).
    """
    office_col = 'office'
    progress_col = 'progress'
...
def process_palika_data_313(df: pd.DataFrame) -> pd.DataFrame:
    """
    Palika beneficiaries = additional_toilets_built * SAN_BENEFICIARY_PER_TOILET
    """
    office_col = 'office'
    progress_col = 'progress'
    toilets_col = 'additional toilets built'
...
# --------------------------------------------------
# Main app logic (원본 흐름 유지, 필요한 분기만 수정)
# --------------------------------------------------
try:
    if main_menu == "3.1 Siddhi Shrestha":
        # -------------------- 3.1.1 --------------------
        ...
        # 🔧 UPDATE: 선택한 지표명을 타이틀에 표시 (우선 커밍순)
    ...
    st.info("💡 **해결 방법:** CSV 헤더(컬럼명)를 확인하고, 데이터 컬럼명이 본 코드의 기대 표준명과 크게 다를 경우 상단의 'CSV 컬럼 확인(디버그)' 옵션을 켜서 실제 컬럼명을 확인한 뒤 매핑 사전에 변형명을 추가해 주세요    st.info("💡 **해결 방법:** CSV 헤더(컬럼명)를 확인하고, 데이터 컬럼명이 본 코드의 기대 표준명과 크게 다를 경우 상단의 'CSV 컬럼 확인(디버그)' 옵션을 켜서 실제 컬럼명을 확인한 뒤 매핑 사전에 변형명을 추가해 주세요.")
